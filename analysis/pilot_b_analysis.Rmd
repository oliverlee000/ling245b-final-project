...
A researcher is planning a study comparing reaction times between two experimental conditions. Which of these changes would increase the study's statistical power?
...
Two researchers are studying the same phenomenon:
...
If you're planning a replication of a study that found a medium-sized effect (say, d = 0.5), but you suspect the original study might have overestimated the effect size, how should this affect your sample size planning?
...
{r}
d.selfpaced <- read_csv('~/Downloads/all_data.csv')
View(d.selfpaced)
...
d.selfpaced |> filter(rt < 10000) |>
  ggplot(mapping = aes(x = rt, fill = condition, color = condition)) + 
...
power_result <- pwr.t.test(
  d = d_value,         # Effect size (Cohen's d)
  sig.level = 0.05,      # Alpha level (0.05 is conventional)
  n=10,      # Desired power (0.80 is conventional)
...
Based on our example data, what is the estimated effect size (Cohen's d)? How would you interpret this effect size in practical terms?

How many participants would we need per group to achieve 80% power with our calculated effect size?

For your replication project, how could you estimate the effect size you expect to find? Would you use the original study's reported effect size, or adjust it?
...
df <- df.raw |>
  filter(condition %in% c("upper_bound_some", "upper_bound_only_some", "lower_bound_some"))
...
Identify the effect size of interest (usually the standardized coefficient)

Determine the appropriate power analysis method for this design
